...
Provenance ties a captured session to the machine that produced it: each machine has an ed25519 key (`agit provenance key`) that signs sessions as they are captured, and `agit provenance verify <session>` self-verifies one. The signature is bound to your git committer identity, so set it once like any git repo (`git config --global user.email you@example.com`); agit does not invent one, and it refuses to snap a session under an unset identity rather than attribute it to nobody. To go from "signed by this key" to "VERIFIED AS you", register the device key with your hub account (`agit identity register <you>`, then paste the block into the hub UI): set your git identity, snap, register, and `agit provenance verify` reports VERIFIED AS. agit's own bookkeeping commits stay authored by `agit@local`, so creating agents never needs a git identity. See [Security](docs/guide/security.md).
