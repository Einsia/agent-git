...
impl Git {
    ...
    pub(crate) fn output(&self, repo: &Repo, args: &[&str], limit: usize) -> Result<Output> {
        self.run(repo.inspection_command(args), limit)
    }

    pub(crate) fn effective_origin(&self, repo: &Repo) -> Result<Output> {
    ...
    fn run(&self, command: Command, limit: usize) -> Result<Output> {
        ...
        self.runtime
            .block_on(output_until(command, limit.min(MAX_OUTPUT), self.deadline))
    }
}
...
mod tests {
    ...
    #[test]
    fn origin_queries_cap_both_streams_and_reap_timeout_and_overflow_children() {
        let root = tempfile::tempdir().unwrap();
        for mode in ["wait", "stdout", "stderr", "complete"] {
            let pid = root.path().join(mode);
            let started = Instant::now();
