...
The `secret keystore` row probes the store `secrets.keystore` selects the way a commit uses it (the OS credential store gets a throwaway entry written, read back and deleted) and unlocks the vault if one exists. A warning there means `agit secrets add` and operations that need existing global rules or an unmigrated dictionary can fail; on a machine with no desktop session, `agit config secrets.keystore file` keeps the key in a private file instead.
