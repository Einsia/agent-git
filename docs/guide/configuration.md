...
| `AGIT_ALLOW_SECRETS` | The visible override for the secret gate. Set to `1` (or `true`/`yes`) to let a commit, push, or snapshot through despite suspected secrets. Disclosed on every use, unlike git's `--no-verify`. See [Security](security.html). |
...
The `agit-hub` server (see [Deploying the hub](deploying-the-hub.html)) reads these. Leave them all
...
| `.agit-allow-secrets` | the agent's store | The secret-scan allowlist: known-safe strings the gate should not flag. See [Security](security.html). |
| `$AGIT_HOME/identity/ed25519` | your machine | The per-machine ed25519 signing key (private key `0600`) that provenance signs sessions with. Minted on first use. See [Security](security.html). |
| `sessions/sync/*.decisions.md` | the agent's store | The merge conflict ledger: the accept, custom, and defer decisions from each `agit a merge`, beside the merge transcript. See [Merging](merging.html). |
...
counterpart to the `.agit-allow-secrets` allowlist. Both are covered under [Security](security.html).
