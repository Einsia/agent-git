...
& cargo test --locked --release --target $target --test windows_rc --test import_noninteractive_selection --test import_lineage_preview --test merge_recon --test merge_settlement --test hub_credential_binding --test whoami_identity_snapshot --test codex_resume_provider --test windows_json_capture -- --nocapture
...
foreach ($suite in @('native_snapshot', 'domain::native_archive::tests', 'adapter::codex_index::tests', 'domain::import_lineage::tests', 'domain::repo::tests::local_', 'commands::fix::tests')) {
